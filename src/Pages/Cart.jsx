...
function Cart() {
  ...
  useEffect(()=>{
    if(cart?.length>0){
      cart?.map(item=>item.totalPrice)?.reduce((p1,p2)=>p1+p2)
      ...
      {cart?.length > 0 ? <div className="container pt-5">
        ...
        <div className="row">
          ...
          <div className='shadow border rounded p-4'>
            <h5>Total Producs : <span className='fw-bolder text-danger'>{cart?.length}</span></h5>
